...
public class DefaultResourceLoader implements ResourceLoader {
	...
	// 策略模式：根据知道的想要的模式，获取对应的 Resource 资源实现实例
	@Override
	public Resource getResource(String location) {
		...
		// 若以 classpath: 开头，则构造 ClassPathResource 类型资源并返回，在构造该资源时，通过 getClassLoader() 获取当前的 ClassLoader
		else if (location.startsWith(CLASSPATH_URL_PREFIX)) {
			return new ClassPathResource(location.substring(CLASSPATH_URL_PREFIX.length()), getClassLoader());
		}
		else {
			// 构造 URL，尝试通过它进行资源定位，若没有抛出 MalformedURLException 异常
			// 先判断是否为 FileURL，如果是则构造 FileUrlResource 类型资源，否则构造 UrlResource
